...
def scanner_photo(request):
    ...
    try:
        DeepFace.detectFace(test_dir) # detector_backend='dlib' but dlib ModuleNotFoundError
    except ValueError:
        shutil.rmtree('input')
        return Response({
    ...
    try:
        ...
        try:
            attendance = Attendance.objects.filter(date=date.today()).get(employee=employee)
            if check:
